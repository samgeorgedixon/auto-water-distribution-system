...
<body>
    ...
    <script>
        ...
        async function RestoreOriginals() {
            ...
            document.getElementById("timeInterval").value = routine.timeInterval / (unitToSeconds[routine.timeIntervalUnit] || 1);
            document.getElementById("timeIntervalUnit").value = routine.timeIntervalUnit;
            ...
            staggerValves.checked = routine.staggerValves;

            // Temp Durations
            const tempContainer = document.getElementById("tempDurationsContainer");
            tempContainer.innerHTML = "";

            for (let i = 0; i < routine.tempRangeDurations.length; i += 2) {
                const li = document.createElement("li");

                CreateLabel(li, "<= Temp: ");
                CreateNumberMinMaxInput(li, routine.tempRangeDurations[i], -40, 80);

                CreateLabel(li, " ≡ Duration: ");
                CreateNumberMinMaxInput(li, routine.tempRangeDurations[i + 1], 0, "");
